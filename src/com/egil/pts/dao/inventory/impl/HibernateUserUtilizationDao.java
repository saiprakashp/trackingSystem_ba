...
@Repository("userUtilizationDao")
public class HibernateUserUtilizationDao extends HibernateGenericDao<UserUtilization, Long>
	...
	public List<Utilization> getUserUtilizationSummary(Long supervisorId, String fromDate, String toDate, Long userId,
		 ...
		 String queryString = "  SELECT resourceType, id,(select ACTIVITY_CODE_NAME from PTS_ACTIVITY_CODES where id=ACTIVITY_CODE_ID) 'activityCodeName' ,coalesce(DATE_FORMAT(WEEKDATE, '%d%m%Y')) parseWeekEndingDate,DATE_FORMAT(WEEKDATE, '%m/%d/%Y') as weekEndingDate,NETWORK_CODE_ID,signum,COALESCE(effort,0.0) 'effort',status,resourceName,networkCodeName,nwStatus,email,supervisorId,(select name from PTS_USER S where S.id = supervisorId) supervisorName FROM (  (SELECT U.id as id,(select USER_TYPE from PTS_USER_ROLE r,PTS_USER_TYPES t where r.TYPE_ID =t.id and r.USER_ID =U.id) resourceType,U.USERNAME 'signum', U.email as email, U.NAME 'resourceName',U.status,S.SUPERVISOR_ID 'supervisorId' FROM PTS_USER U JOIN PTS_USER_SUPERVISOR S ON S.USER_ID=U.ID AND U.status not in ('OffBoard','No Show','Open','Selected','Deleted','Induction'))A  LEFT OUTER JOIN   (SELECT N.STATUS 'nwStatus',concat(N.RELEASE_ID, ' - ', N.RELEASE_NAME)'networkCodeName',T.ACTIVITY_CODE_ID,T.WEEKENDING_DATE 'WEEKDATE',T.NETWORK_CODE_ID,coalesce(sum((T.mon_hrs + T.tue_hrs + T.wed_hrs + T.thu_hrs + T.fri_hrs + T.sat_hrs + T.sun_hrs)),0.0) as effort,T.USER_ID FROM  PTS_USER_TIMESHEET T,PTS_NETWORK_CODES N WHERE T.NETWORK_CODE_ID=N.ID GROUP BY  T.NETWORK_CODE_ID,T.USER_ID,T.WEEKENDING_DATE ) TIM  ON A.id=TIM.USER_ID  )WHERE  UPPER(signum) NOT LIKE '%ADMIN%' AND status not in ('OffBoard','No Show','Open','Selected','Deleted','Induction') &super &user &tmNw &tmStab &projectManager &weekCond &del  GROUP BY  networkCodeName,ID,WEEKDATE   ";
		    ...
		    if (networkCodeId != null && networkCodeId.longValue() > 0L) {
		    ...
		    if (projectManagerId != null && projectManagerId.longValue() > 0L) {
